...
div
  el-dialog.err-dialog(title='Error message' :visible.sync="dialogVisible" :close-on-click-modal="false")
    editor.exception-message(:value="code")

  el-row.empty-container(v-if="running.length === 0")
    el-row.icon-container(type="flex" justify="center")
      svg-icon(icon-class='beach' class-name='empty-icon')
    el-row.empty-text(type="flex" justify="center")
      span No running sessions.
  ...
  el-table(
    ...
    el-table-column(label='Duration' min-width='200')
      template(slot-scope='scope')
        | {{duration(scope.row.startTime)}}
    el-table-column(label='Code' max-width='20' align="center")
      template(slot-scope='scope')
        i.el-icon-more(@click="onCode(scope.row.code)")
</template>
...
export default {
  ...
  methods: {
    ...
    duration(startTime) {
      ...
      const d = moment.duration(this.now - moment(startTime))
