...
class MailService {
  ...
  def test(s: MailSetting) = {
    ...
    m.to = s.to
    m.from = s.from
    m.subject= "Test mail from alchemist"
  ...
  def sendUserPasswordMail(user: User, password: String) = {
    ...
    if(s != None && s.get.isActive){
      if(user.email != null){
        mail.to = user.email
        mail.subject= s"${user.name} welcome to Alchemist"
  ...
  def sendSessionBlockMail(message: String) = {
    settingService.findMailSettings match {
      case Some(s) =>
        ...
        mail.to = s.to
        mail.from = s.from
        mail.subject= s"Blocking sessions - Alchemist"
        mail.body = message
        sendMail(mail, s)
      case _ =>
    }
  ...
  def sendMail(m: Mail, s: MailSetting) {
    ...
    log.debug("Mail server OK")
    try{
      val message = new MimeMessage(session)
      message.setFrom(new InternetAddress(if(m.from!=null) m.from else s.from))

      m.to = if(m.to == null || m.to.trim.isEmpty) s.to else m.to

      m.to.split(",").foreach{m =>
        message.addRecipient(Message.RecipientType.TO,new InternetAddress(m.trim))
      }
